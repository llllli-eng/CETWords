...
5. 点击“保存并测试连接”。连接成功后，新词“当日巩固”可使用中文释义输入；今日任务完成后还可主动生成一次 AI 每日复盘。复盘不会自动请求。
...
- 中文释义判题每次只发送当前单词的词性、标准释义和本次回答。
- 每日复盘只发送当日聚合统计、最多 10 个薄弱词与 5 个纠正词，不发送原始回答、整份词库、例句库、真题正文或完整历史。
