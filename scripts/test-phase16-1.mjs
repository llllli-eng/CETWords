...
await test("v12 to v15 preserves the old target's local date", () => {
...
await test("v15 schedules stay identical across save and reload", () => {
  const first = loadApp();
...
await test("ordinary practice cannot move an existing v15 long-term anchor", () => {
  const first = loadApp();
...
await test("backup validation accepts every version from v1 through v15", () => {
